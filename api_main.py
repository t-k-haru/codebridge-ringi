...
def inbox(user=Depends(require_roles("manager", "admin"))):
...
def approve(request_id: int, req: ResolveReq, user=Depends(require_roles("manager", "admin"))):
...
def reject(request_id: int, req: ResolveReq, user=Depends(require_roles("manager", "admin"))):
    ringi = auth.get_request(request_id)
    if not ringi:
        raise HTTPException(404, "Not found")
    if ringi["approver_id"] != user["id"]:
...
def update_settings(uid: int, req: ApproverSettingsReq, user=Depends(require_roles("admin"))):
...
@app.get("/api/cost")
def get_cost(user=Depends(require_roles("admin"))):
